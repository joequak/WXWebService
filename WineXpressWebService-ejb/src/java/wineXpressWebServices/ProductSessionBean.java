...
//    }
    ...
    @Override
    public long saveNewProduct(String picture,String productName, double productPrice, double productCost, String productDescription, int productAQ, int productDiscount, String productVolume , String subCategoryName) {
        Query pq = em.createQuery("select p from Product p where p.name=:pName");
        pq.setParameter("pName", productName);
        if(!pq.getResultList().isEmpty()){
            return -2l;//product existed;
        }
        ...
        em.persist(product);
        Query q = em.createQuery("select s from SubCategories s where s.name=:subCategoryName");
        q.setParameter("subCategoryName", subCategoryName);
        SubCategories sub  = (SubCategories)q.getResultList().get(0);
        sub.getProductCollection().add(product);
        em.persist(sub);
